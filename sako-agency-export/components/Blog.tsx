...
export default function Blog() {
  ...
  return (
    <section ref={sectionRef} id="blog" style={{ background: "var(--black-2)", position: "relative" }}>
      <div className="container section-pad">
        {/* Header */}
        <div style={{ textAlign: "center", marginBottom: "72px" }}>
          <span className="eyebrow" style={{ display: "block", marginBottom: "20px" }}>— Knowledge Hub</span>
          ...
          <p style={{ maxWidth: "520px", margin: "24px auto 0", fontSize: "16px", color: "var(--cream-dim)", lineHeight: 1.8 }}>
            Stay ahead of the curve with fresh content on code, design, startups, and everything in between.
          </p>
        </div>

        {/* Blog Grid */}
        <div className="blog-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "40px" }}>
          {posts.map((post, i) => (
            <div key={i} className="blog-card group" style={{ cursor: "none" }}>
              <div style={{ position: "relative", overflow: "hidden", borderRadius: "16px", aspectRatio: "16/10", marginBottom: "24px", border: "1px solid rgba(201,168,76,0.06)", transition: "border-color 0.4s" }} className="group-hover:border-[rgba(201,168,76,0.2)]">
                <img 
                  src={post.image} 
                  alt={post.title} 
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <span style={{ fontFamily: "'Syne',sans-serif", fontSize: "12px", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--gold)", opacity: 0.8 }}>{post.category}</span>
                <span style={{ fontSize: "13px", color: "rgba(245,240,232,0.4)", fontFamily: "'Outfit', sans-serif" }}>{post.date}</span>
              </div>
              
              <h3 style={{ fontFamily: "'Cormorant',serif", fontSize: "28px", fontWeight: 400, color: "var(--cream)", lineHeight: 1.2, marginBottom: "20px", transition: "color 0.4s" }} className="group-hover:text-[var(--gold)]">
                {post.title}
              </h3>
              
              <button 
                data-cursor="magnetic"
                style={{ display: "flex", alignItems: "center", gap: "10px", background: "none", border: "none", color: "var(--cream-dim)", fontSize: "11px", fontFamily: "'Syne',sans-serif", textTransform: "uppercase", letterSpacing: "0.15em", cursor: "none", transition: "color 0.3s" }} 
        ...
        <div style={{ marginTop: "64px", textAlign: "center" }}>
          <button 
            data-cursor="magnetic"
            style={{ display: "inline-flex", alignItems: "center", gap: "10px", background: "transparent", color: "var(--gold)", fontFamily: "'Syne',sans-serif", fontWeight: 600, fontSize: "11px", letterSpacing: "0.15em", textTransform: "uppercase", padding: "16px 32px", borderRadius: "100px", border: "1px solid rgba(201,168,76,0.3)", cursor: "none", transition: "all 0.4s" }}
